...
class InstagramScraper(object):
	...
	def __init__(self, **kwargs):
		default_attr = dict(username='', usernames=[], filename=None,
		                    ...
		                    template='{urlname}', log_destination='', save_pagination_index=False)
	...
	def get_dst_dir(self, username):
		"""Gets the
         directory and last scraped file time."""
		if self.destination == './':
			dst = './' + username
		else:
	...
	def augment_node(self, node):
		...
		if self.save_pagination_index:
			return node['taken_at_timestamp']
	...
	def get_profile_pic(self, dst, executor, future_to_item, user, username):
		...
		item = {
			'urls': [profile_pic_url], 'username': username, 'shortcode': '', 'created_time': 1286323200,
		...
		if resp is not None:
			...
			for broadcast in retval['post_live_item']['broadcasts']:
				...
				video_element = video_adaptation_set.find(
					'.//*[@width="{0}"][@height="{1}"]/{2}BaseURL'.format(best_video_quality[0], best_video_quality[1],
					                                                      xmlns))
...
def main():
	...
	parser.add_argument('--media-types', '--media_types', '-t', nargs='+',
	                    default=['image', 'video', 'story', 'broadcast'],
	...
	parser.add_argument('--save-pagination-index', '--save_pagination_index', action='store_true', default=False,
	                    help='Build pagination index for a tag or location')
...
def test():
	scraper = InstagramScraper(include_location=False, verbodse=0, username=['covid'], tag=True, maximum=200,
	                           media_types='none', save_pagination_indexe=True, retry_forever=True)
